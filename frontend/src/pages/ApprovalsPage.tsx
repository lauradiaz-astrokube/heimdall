...
function RequestCard({ request }: { request: AccessRequest }) {
  ...
  if (done) {
    return (
      <div className="hcard p-4 sm:p-5" style={{
        borderLeftWidth: "3px",
        borderLeftColor: result === "approved" ? "#10b981" : "#ef4444"
      }}>
        <p className="text-sm font-semibold htext-1">{result === "approved" ? "Acceso concedido" : "Solicitud rechazada"}</p>
    ...
    <div className="hcard overflow-hidden">
      ...
      <div className="p-4 sm:p-5 flex flex-col gap-4">
        ...
        <div>
          ...
          <p className="text-xs font-mono htext-3 mt-1">
        ...
        <div className="rounded-lg px-4 py-3" style={{ backgroundColor: "var(--bg-deep)", border: "1px solid var(--border)" }}>
          <p className="text-[10px] font-bold uppercase tracking-widest htext-3 mb-1.5">Justificación</p>
          <p className="text-sm htext-1">{request.justification}</p>
        </div>

        {/* Comentario */}
        <input type="text" placeholder="Comentario opcional..." className="hinput"
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          <button onClick={() => { setError(null); approveMutation.mutate({ id: request.id, comment }); }} disabled={isPending}
            className="hbtn-success flex-1">
            {approveMutation.isPending ? "Aprobando..." : "Aprobar"}
          </button>
          <button onClick={() => { setError(null); rejectMutation.mutate({ id: request.id, comment }); }} disabled={isPending}
            className="hbtn-danger flex-1">
...
export function ApprovalsPage() {
  ...
  return (
    <Layout back title="Aprobaciones">
      <div className="mx-auto w-full max-w-2xl">
        <div className="mb-6 sm:mb-7">
          <h1 className="text-xl sm:text-2xl font-black tracking-tight htext-1">Solicitudes pendientes</h1>
          <p className="mt-1.5 text-sm htext-2">Revisa y valida las solicitudes de acceso temporal del equipo.</p>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:gap-4">
          ...
          {!isLoading && (!requests || requests.length === 0) && (
            <div className="rounded-xl border border-dashed p-10 text-center" style={{ borderColor: "var(--border)" }}>
              <p className="text-sm htext-3">No hay solicitudes pendientes</p>
            </div>
          )}
